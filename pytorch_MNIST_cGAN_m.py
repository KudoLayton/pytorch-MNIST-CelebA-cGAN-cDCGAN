...
def FID_check():
    ...
    test_loader = [(_x, _y) for _x, _y in test_loader][:10]
    fid = []
    for x_, y_ in test_loader:
        y_label_ = torch.zeros(100, 10)
        y_label_.scatter_(1, y_.view(100, 1), 1)
        x_ = x_.view(-1, 28 * 28)
        x_, y_label_ = Variable(x_.cuda()), Variable(y_label_.cuda())

        z_ = torch.randn((100, 100))
        y_ = (torch.rand(100, 1) * 10).type(torch.LongTensor)

        z_ = Variable(z_.cuda())

        G_result = G(z_, y_label_).cpu().data.numpy()
        real_result = x_.cpu().data.numpy()

        m = np.mean(G_result, axis=0)
        c = np.cov(np.transpose(G_result))

        mw = np.mean(real_result, axis=0)
        cw = np.cov(np.transpose(real_result))

        fid.append(np.linalg.norm(m-mw) + np.trace(c + cw - 2 * (np.sqrt(c * cw))))
    print('average fid is %f'%(sum(fid) / 10))
